...
>
> 1. `reactive`重新分配一个新对象，会**失去**响应式（可以使用<font color="#00b0f0">`Object.assign`</font>去整体替换）。
> 	1. 更改reactive的对象

- ref和reactive使用原则：
> 2. 若需要一个基本类型的响应式数据，必须使用`ref`。
> 3. 若需要一个响应式对象，层级不深，`ref`、`reactive`都可以。
> 4. 若需要一个响应式对象，且层级较深，推荐使用`reactive`。
...
1. 若该属性值**不是**【对象类型】，需要写成<font color="#00b0f0">函数形式</font>。
